...
class AvatarImageView : AppCompatImageView {
    ...
    constructor(context: Context, attrs: AttributeSet, defStyle: Int = 0) : super(context, attrs, defStyle) {
        ...
        mBorderWidth = a.getDimensionPixelSize(R.styleable.AvatarImageView_borderWidth, DEFAULT_BORDER_WIDTH)
        mBorderColor = a.getColor(R.styleable.AvatarImageView_borderColor, DEFAULT_BORDER_COLOR)
        mBorderOverlay = a.getBoolean(R.styleable.AvatarImageView_borderOverlay, DEFAULT_BORDER_OVERLAY)

        if (a.hasValue(R.styleable.AvatarImageView_circleBackgroundColor)) {
            mCircleBackgroundColor = a.getColor(R.styleable.AvatarImageView_circleBackgroundColor,
                    DEFAULT_CIRCLE_BACKGROUND_COLOR)
        } else if (a.hasValue(R.styleable.AvatarImageView_fillColor)) {
            mCircleBackgroundColor = a.getColor(R.styleable.AvatarImageView_fillColor,
                    DEFAULT_CIRCLE_BACKGROUND_COLOR)
        }
    ...
    fun loadHttpImage(url:String){
        require(!url.isNullOrEmpty() && url.startsWith("http")){
            "Invalid url : $url"
        }
        AvatarLoader.with(context).load(this,url)
    }

    fun loadHttpImage(url:String,drawable:Int){
        require(!url.isNullOrEmpty() && url.startsWith("http")){
            "Invalid url : $url"
        }
        AvatarLoader.with(context).load(this,url,drawable)
    }
